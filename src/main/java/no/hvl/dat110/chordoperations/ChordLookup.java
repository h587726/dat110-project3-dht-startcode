...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		NodeInterface stubNodeInterface = Util.getProcessStub(succNodeInterface.getNodeName(),
				succNodeInterface.getPort());
		
		// if logic returns true, then return the successor
		if(Util.checkInterval(key, node.getNodeID().add(new BigInteger("1")),
				stubNodeInterface.getNodeID())){
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for (int i = fingerTable.size() - 1; i >= 0; i--) {
			...
			// check that finger is a member of the set
			// {nodeID+1,...,ID-1} i.e. (nodeID+1 <= finger <= key-1) using the ComputeLogic
			if(Util.checkInterval(stub.getNodeID(),
					node.getNodeID().add(new BigInteger("1")),
